# Split Flap F1 — Flap Allocation v4 (52 Flaps)

## 5 Columns × 5 Rows, 52 Flaps Per Unit

---

## Defaults

- **Background:** Black (#000000)
- **Font color:** White (#FFFFFF)
- **Font:** Roboto Mono Bold, 36pt (numbers, letters, symbols)
- **Small text font:** Roboto Condensed Bold, 24pt (mph, kph, compound labels)
- **Icons/Logos:** Custom vector artwork, fit to flap with padding
- Tables only note BG/FG where they **differ** from defaults.

---

## Position Alignment

| Range | Alignment |
|-------|-----------|
| 0–7 | ✅ ALL columns — 8 base colors |
| 8–17 | ✅ ALL columns — numbers 0–9 |
| 18–43 | ✅ Col 1, 2, 3, 4 — letters A–Z (Col 1 starts at 18, Cols 2–4 also start at 18) |
| 18+ | Col 0 diverges (logos + flags) |

---

## SHARED: All 5 Columns (Pos 0–17)

| Pos | Content | BG | Notes |
|-----|---------|----|----|
| 0 | Black (blank) | — | Home position |
| 1 | Red | #e40810 | Ferrari, red flag, soft |
| 2 | Orange | #FF8000 | McLaren |
| 3 | Yellow | #ffd032 | Yellow flag, SC, medium |
| 4 | White | #FFFFFF | Hard, Haas |
| 5 | Green | #04c009 | Aston Martin, green flag, inter, sector PB |
| 6 | Blue | #011cd2 | Red Bull, blue flag, wet |
| 7 | Purple | #b204b3 | Racing Bulls, sector best, FL |
| 8 | 0 | — | |
| 9 | 1 | — | |
| 10 | 2 | — | |
| 11 | 3 | — | |
| 12 | 4 | — | |
| 13 | 5 | — | |
| 14 | 6 | — | |
| 15 | 7 | — | |
| 16 | 8 | — | |
| 17 | 9 | — | |

**0–7:** 8 base colors (warm→cool)
**8–17:** Digits 0–9

---

## Col 0: Identity — Logos + Flags (52 flaps)

| Pos | Content | BG | FG | Type | Notes |
|-----|---------|----|----|------|-------|
| 0 | Black | — | — | solid | Home |
| 1 | Red | #e40810 | — | solid | |
| 2 | Orange | #FF8000 | — | solid | |
| 3 | Yellow | #ffd032 | — | solid | |
| 4 | White | #FFFFFF | — | solid | |
| 5 | Green | #04c009 | — | solid | |
| 6 | Blue | #011cd2 | — | solid | |
| 7 | Purple | #b204b3 | — | solid | |
| 8 | 0 | — | — | digit | |
| 9 | 1 | — | — | digit | |
| 10 | 2 | — | — | digit | |
| 11 | 3 | — | — | digit | |
| 12 | 4 | — | — | digit | |
| 13 | 5 | — | — | digit | |
| 14 | 6 | — | — | digit | |
| 15 | 7 | — | — | digit | |
| 16 | 8 | — | — | digit | |
| 17 | 9 | — | — | digit | |
| 18 | Ferrari logo |  | — | logo | Oldest — 1950 |
| 19 | McLaren logo |  | — | logo | 1966 |
| 20 | Williams logo | | — | logo | 1977 |
| 21 | Red Bull logo | | — | logo | 2005 |
| 22 | Mercedes logo | | | logo | 2010 works — black logo on teal |
| 23 | Haas logo | |  | logo | 2016 — black logo on white |
| 24 | Alpine logo |  | | logo | 2021 — black logo on pink |
| 25 | Aston Martin logo |  | — | logo | 2021 |
| 26 | Racing Bulls logo | | — | logo | 2024 |
| 27 | Audi logo |  | — | logo | 2026 |
| 28 | Cadillac logo | |  | logo | Newest — 2026 new entry, black logo on gold |
| 29 | Thailand flag | — | — | flag | Future / proposed calendar addition |
| 30 | Germany flag | — | — | flag | Off-calendar, possible return — Hockenheim / Nürburgring |
| 31 | USA flag | — | — | flag | Las Vegas 2023 (Austin/Miami/Vegas) |
| 32 | Saudi Arabia flag | — | — | flag | Jeddah — 2021 |
| 33 | Qatar flag | — | — | flag | Lusail — 2021 |
| 34 | Netherlands flag | — | — | flag | Zandvoort — 2021 return |
| 35 | Azerbaijan flag | — | — | flag | Baku — 2016 |
| 36 | Mexico flag | — | — | flag | Mexico City — 2015 return |
| 37 | Austria flag | — | — | flag | Red Bull Ring — 2014 return |
| 38 | UAE flag | — | — | flag | Abu Dhabi — 2009 |
| 39 | Singapore flag | — | — | flag | Marina Bay — 2008 |
| 40 | Bahrain flag | — | — | flag | Sakhir — 2004 |
| 41 | China flag | — | — | flag | Shanghai — 2004 |
| 42 | Australia flag | — | — | flag | Melbourne — 1996 |
| 43 | Spain flag | — | — | flag | Barcelona — 1991 |
| 44 | Japan flag | — | — | flag | Suzuka — 1987 |
| 45 | Hungary flag | — | — | flag | Hungaroring — 1986 |
| 46 | Canada flag | — | — | flag | Montreal — 1978 |
| 47 | Brazil flag | — | — | flag | Interlagos — 1973 |
| 48 | Belgium flag | — | — | flag | Spa — 1950 |
| 49 | UK flag | — | — | flag | Silverstone — 1950 |
| 50 | Italy flag | — | — | flag | Monza — 1950 |
| 51 | Monaco flag | — | — | flag | Monte Carlo — oldest, 1950 |

**Total: 8 colors + 10 numbers + 11 logos + 23 flags = 52**

---

## Col 1: Status / Info — Weather, Icons, Letters (52 flaps)

| Pos | Content | BG | FG | Type | Notes |
|-----|---------|----|----|------|-------|
| 0 | Black | — | — | solid | Home |
| 1 | Red | #e40810 | — | solid | |
| 2 | Orange | #FF8000 | — | solid | |
| 3 | Yellow | #ffd032 | — | solid | |
| 4 | White | #FFFFFF | — | solid | |
| 5 | Green | #04c009 | — | solid | |
| 6 | Blue | #011cd2 | — | solid | |
| 7 | Purple | #b204b3 | — | solid | |
| 8 | 0 | — | — | digit | |
| 9 | 1 | — | — | digit | |
| 10 | 2 | — | — | digit | |
| 11 | 3 | — | — | digit | |
| 12 | 4 | — | — | digit | |
| 13 | 5 | — | — | digit | |
| 14 | 6 | — | — | digit | |
| 15 | 7 | — | — | digit | |
| 16 | 8 | — | — | digit | |
| 17 | 9 | — | — | digit | |
| 18 | A | — | — | letter | |
| 19 | B | — | — | letter | |
| 20 | C | — | — | letter | |
| 21 | D | — | — | letter | |
| 22 | E | — | — | letter | |
| 23 | F | — | — | letter | |
| 24 | G | — | — | letter | |
| 25 | H | — | — | letter | |
| 26 | I | — | — | letter | |
| 27 | J | — | — | letter | |
| 28 | K | — | — | letter | |
| 29 | L | — | — | letter | |
| 30 | M | — | — | letter | |
| 31 | N | — | — | letter | |
| 32 | O | — | — | letter | |
| 33 | P | — | — | letter | |
| 34 | Q | — | — | letter | |
| 35 | R | — | — | letter | |
| 36 | S | — | — | letter | |
| 37 | T | — | — | letter | |
| 38 | U | — | — | letter | |
| 39 | V | — | — | letter | |
| 40 | W | — | — | letter | |
| 41 | X | — | — | letter | |
| 42 | Y | — | — | letter | |
| 43 | Z | — | — | letter | Full A–Z alphabet |
| 44 | Sun | #FFD700 | — | icon | Weather: clear |
| 45 | Partly Cloudy | — | — | icon | Weather: partly cloudy |
| 46 | Cloud | #808080 | — | icon | Weather: overcast |
| 47 | Rain | #1E41FF | — | icon | Weather: rain |
| 48 | Thunder | — | — | icon | Weather: thunderstorm |
| 49 | Snow | — | — | icon | Weather: snow |
| 50 | Wind | — | — | icon | Weather: wind |
| 51 | Thermometer | — | — | icon | Temperature indicator |

**Total: 8 colors + 10 numbers + 26 letters (full A–Z) + 8 icons = 52**

---

## Col 2: Text / Numbers / Colors (52 flaps)

| Pos | Content | BG | FG | Type | Notes |
|-----|---------|----|----|------|-------|
| 0–7 | (colors) | (shared) | — | solid | 8 base colors |
| 8–17 | (digits) | — | — | digit | 0–9 |
| 18 | A | — | — | letter | |
| 19 | B | — | — | letter | |
| 20 | C | — | — | letter | |
| 21 | D | — | — | letter | |
| 22 | E | — | — | letter | |
| 23 | F | — | — | letter | |
| 24 | G | — | — | letter | |
| 25 | H | — | — | letter | |
| 26 | I | — | — | letter | |
| 27 | J | — | — | letter | |
| 28 | K | — | — | letter | |
| 29 | L | — | — | letter | |
| 30 | M | — | — | letter | |
| 31 | N | — | — | letter | |
| 32 | O | — | — | letter | |
| 33 | P | — | — | letter | |
| 34 | Q | — | — | letter | |
| 35 | R | — | — | letter | |
| 36 | S | — | — | letter | |
| 37 | T | — | — | letter | |
| 38 | U | — | — | letter | |
| 39 | V | — | — | letter | |
| 40 | W | — | — | letter | |
| 41 | X | — | — | letter | |
| 42 | Y | — | — | letter | |
| 43 | Z | — | — | letter | |
| 44 | - | — | — | text | Dash |
| 45 | . | — | — | text | Period |
| 46 | ! | — | — | text | Exclamation |
| 47 | ° | — | — | text | Degree symbol |
| 48 | % | — | — | text | Percent |
| 49 | + | — | — | text | Plus |
| 50 | / | — | — | text | Slash |
| 51 | : | — | — | text | Colon |

**Total: 8 colors + 10 numbers + 26 letters + 8 symbols = 52**

---

## Col 3: Text / Numbers / Colors (52 flaps)

| Pos | Content | BG | FG | Type | Notes |
|-----|---------|----|----|------|-------|
| 0–7 | (colors) | (shared) | — | solid | 8 base colors |
| 8–17 | (digits) | — | — | digit | 0–9 |
| 18–43 | A–Z | — | — | letter | Full alphabet |
| 44 | - | — | — | text | Dash |
| 45 | . | — | — | text | Period |
| 46 | ! | — | — | text | Exclamation |
| 47 | ° | — | — | text | Degree symbol |
| 48 | % | — | — | text | Percent |
| 49 | + | — | — | text | Plus |
| 50 | / | — | — | text | Slash |
| 51 | : | — | — | text | Colon |

**Total: 8 colors + 10 numbers + 26 letters + 8 symbols = 52**

---

## Col 4: Text / Numbers / Colors (52 flaps)

| Pos | Content | BG | FG | Type | Notes |
|-----|---------|----|----|------|-------|
| 0–7 | (colors) | (shared) | — | solid | 8 base colors |
| 8–17 | (digits) | — | — | digit | 0–9 |
| 18–43 | A–Z | — | — | letter | Full alphabet |
| 44 | - | — | — | text | Dash |
| 45 | . | — | — | text | Period |
| 46 | ! | — | — | text | Colon |
| 47 | ° | — | — | text | Degree symbol |
| 48 | % | — | — | text | Percent |
| 49 | + | — | — | text | Plus |
| 50 | mph | — | — | small text | Wind speed (imperial) |
| 51 | kph | — | — | small text | Wind speed (metric) |

**Total: 8 colors + 10 numbers + 26 letters + 6 symbols + 2 unit labels = 52**

---

## CROSS-COLUMN ALIGNMENT SUMMARY

| Pos | Col 0 | Col 1 | Col 2 | Col 3 | Col 4 |
|-----|-------|-------|-------|-------|-------|
| 0–7 | 8 Colors | 8 Colors | 8 Colors | 8 Colors | 8 Colors |
| 8–17 | Numbers | Numbers | Numbers | Numbers | Numbers |
| 18–43 | Logos (11) + Flags (15) | Letters A–Z (26) | Letters A–Z (26) | Letters A–Z (26) | Letters A–Z (26) |
| 44–51 | Flags (8) | Weather icons (8) | Symbols (8) | Symbols (8) | Symbols (6) + mph + kph |

**Col 0 totals: 8 colors + 10 numbers + 11 logos + 23 flags = 52**

---

## DISPLAY EXAMPLES

### Driver Standings
```
[Col0]            [Col1]     [Col2] [Col3] [Col4]
[Ferrari logo]    [ 1 ]      [ N ]  [ O ]  [ R ]
[McLaren logo]    [ 2 ]      [ P ]  [ I ]  [ A ]
[Red Bull logo]   [ 3 ]      [ V ]  [ E ]  [ R ]
[Mercedes logo]   [ 4 ]      [ H ]  [ A ]  [ M ]
[Aston logo]      [ 5 ]      [ A ]  [ L ]  [ O ]
```

### Constructor Standings
```
[Col0]            [Col1]     [Col2] [Col3] [Col4]
[McLaren logo]    [ 1 ]      [ M ]  [ C ]  [ L ]
[Ferrari logo]    [ 2 ]      [ F ]  [ E ]  [ R ]
[Red Bull logo]   [ 3 ]      [ R ]  [ B ]  [ R ]
[Mercedes logo]   [ 4 ]      [ M ]  [ E ]  [ R ]
[Aston logo]      [ 5 ]      [ A ]  [ M ]  [ R ]
```

### Weather Display
```
[Col0]            [Col1]         [Col2] [Col3] [Col4]
[Monaco flag]     [Location Pin] [ M ]  [ O ]  [ N ]
[Clock]           [1]            [ 3 ]  [ 3 ]  [ 0 ]
[  ]              [Rain]         [ 1 ]  [ 0 ]  [ % ]
[  ]              [Wind]         [ N ]  [ W ]  [   ]
[  ]              [Wind]         [ 1 ]  [ 5 ]  [kph]
```

### Practice Results (with gaps)
```
[Col0]            [Col1]       [Col2] [Col3] [Col4]
[Ferrari logo]    [ 1 ]       [ L ]  [ E ]  [ C ]
[McLaren logo]    [ 2 ]       [ 0 ]  [ . ]  [ 3 ]
[Red Bull logo]   [ 3 ]       [ 0 ]  [ . ]  [ 7 ]
[Mercedes logo]   [ 4 ]       [ 1 ]  [ . ]  [ 1 ]
[Aston logo]      [ 5 ]       [ 1 ]  [ . ]  [ 4 ]
```

### Upcoming Race Info
```
[Col0]            [Col1]         [Col2] [Col3] [Col4]
[UK flag]         [Stopwatch]    [ S ]  [ I ]  [ L ]
[Clock]           [ 1 ]          [ 4 ]  [ 0 ]  [ 0 ]
[  ]              [Sun]          [ 2 ]  [ 8 ]  [ ° ]
[  ]              [Rain]         [ 1 ]  [ 5 ]  [ % ]
[  ]              [Wind]         [ 1 ]  [ 2 ]  [kph]
```

---

## TOTAL BUILD NUMBERS

| Metric | Value |
|--------|-------|
| Grid | 5 columns × 5 rows |
| Units | 25 |
| Flaps per unit | 52 |
| Total flaps | **1,300** |
| Unique drum types | **4** (Col 0, Col 1, Col 2/3 shared, Col 4) |
| Col 0 drums | 5 |
| Col 1 drums | 5 |
| Col 2–3 drums | 10 (identical) |
| Col 4 drums | 5 |
| Steps per revolution | 2048 |
| Steps per flap | ~39.4 (use lookup table) |

---

## MANUFACTURING SPEC

### Defaults (unless overridden)

| Property | Value |
|----------|-------|
| Background | Black #000000 full bleed |
| Foreground | White #FFFFFF |
| Font (digits/letters/symbols) | TODO |
| Font (small labels: mph, kph) | TODO |
| Icons | Custom vector, white on black unless noted - TODO |
| Logos | Team vector art on team-color BG TODO |
| Flags | Full-color country flag artwork TODO |
| Finish | Matte |

### Print Types

| Type | Description |
|------|-------------|
| solid | Full-bleed background color, no text |
| digit | Number character (default BG/FG) |
| letter | Letter character (default BG/FG) |
| text | Symbol character (default BG/FG) |
| small text | Smaller font label (mph, kph) |
| icon | Vector graphic on colored or default BG |
| logo | Team logo on team-color BG |
| flag | Full-color country flag artwork |
